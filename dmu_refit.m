...
for i = [1:3]
    E_min = E_min_init;
    E_max = E_max_init;
    E_min_init = E_min_init + 0.01;
    E_max_init = E_max_init + 0.01;
...
for i3 = 2:length(Int)
    y = ratio{1,i3}(Einv(E_min):Einv(E_max));
    ...
    beta0_dmu_init{i3} = [T(i,i3);mu(i,i3)-mu_ref(i);mu_ref(i)];
%     beta0_dmu_init{i3} = [T(i);mu(i)-mu_ref_init(i);mu_ref_init(i)];
    [beta_dmu_init{i3},r,J,cov] = nlinfit(x,y,modelfun,beta0_dmu_init{i3},options);
    T_dmu_init(i3) = beta_dmu_init{i3}(1,1);
    delta_mu_dmu_init(i3) = beta_dmu_init{i3}(2,1);
    mu_dmu_init(i3) = beta_dmu_init{i3}(2,1)+beta_dmu_init{i3}(3,1);
    mu_ref_dmu_init(i,i3) = beta_dmu_init{i3}(3,1);
    ci_dmu_init{i3} = nlparci(beta_dmu_init{i3},r,'covar',cov);
    cj_dmu_init{i3} = nlparci(beta_dmu_init{i3},r,'Jacobian',J);
    mu_ref_inv_var(i3) = 4/(ci_dmu_init{i3}(3,2)-ci_dmu_init{i3}(3,1));
    mu_top(i3) =  mu_ref_dmu_init(i,i3).*(mu_ref_inv_var(i3).^2);
end
 mu_ref_dmu_init(i,1)=NaN;
 mu_ref_dmu(i) = sum(mu_top)./sum((mu_ref_inv_var.^2));
 var_mu_ref_dmu(i) = 2.*sqrt(1./sum((mu_ref_inv_var.^2)));
...
for i4 = 1:length(Int)
    y = ratio{1,i4}(Einv(E_min):Einv(E_max));
    x = E(Einv(E_min):Einv(E_max));
    modelfun = @(a,x) dmu_T_ratio(a(1), a(2), mu_ref_dmu(i),x,T_L,m_e,m_h,Eg,D);
    options = statset('FunValCheck','off','DerivStep',10^-12,'robust','on');
    beta0_dmu{i4} = [T_dmu_init(i4);delta_mu_dmu_init(i4)];
    [beta_dmu{i4},r,J,cov] = nlinfit(x,y,modelfun,beta0_dmu{i4},options);
    T_dmu(i,i4) = beta_dmu{i4}(1,1);
    delta_mu_dmu(i,i4) = beta_dmu{i4}(2,1);
    mu_nfit_dmu(i,i4) = beta_dmu{i4}(2,1)+mu_ref_dmu(i);
    ci_dmu{i,i4} = nlparci(beta_dmu{i4},r,'covar',cov);
    cj_dmu{i,i4} = nlparci(beta_dmu{i4},r,'Jacobian',J);
    T_error_dmu(i,i4) = (ci_dmu{i,i4}(1,2)-ci_dmu{i,i4}(1,1))/2;
    delta_mu_error_dmu(i,i4) = (ci_dmu{i,i4}(2,2)-ci_dmu{i,i4}(2,1))/2;
end
...
for i=1:length(Int)
    T_dmu_raw(i) = T_dmu(2,i);
    T_plus_dmu(i) = T_dmu(3,i);
    T_min_dmu(i) = T_dmu(1,i);
    delta_T_interval_dmu(i) = max(abs(T_plus_dmu(i)-T_dmu_raw(i)),abs(T_dmu_raw(i)-T_min_dmu(i)));
    delta_T_init(i) = T_error_dmu(2,i);
    delta_T_dmu(i) = delta_T_interval_dmu(i)+delta_T_init(i);
    
    dmu_dmu(i) = delta_mu_dmu(2,i);
    dmu_plus_dmu(i) = delta_mu_dmu(3,i);
    dmu_min_dmu(i) = delta_mu_dmu(1,i);
    delta_dmu_interval_dmu(i) = max(abs(dmu_plus_dmu(i)-dmu_dmu(i)),abs(dmu_dmu(i)-dmu_min_dmu(i)));
    delta_mu_dmu_init(i) = delta_mu_error_dmu(2,i);
    delta_dmu_dmu(i) = delta_dmu_interval_dmu(i)+delta_mu_dmu_init(i);
end
...
% Linear fit
p_dmu1 = polyfit(T_dmu_raw-T_L,P_abs,1); %Temperature fit
Q_dmu_nfit = p_dmu1(1);
deltaT0_dmu_nift = p_dmu1(2);
deltaT_dmu_nfitdata = deltaT0_dmu_nift + Q_dmu_nfit*(T_dmu_raw-T_L);

p_dmu2 = polyfit(log(P_abs),dmu_dmu,1); %mu fit
...
errorbar(P_abs,T_dmu_raw-T_L,delta_T_dmu,delta_T_dmu,[],[], '+', 'markerSize',10,'color', colors(1,:),'LineWidth', 3);
plot(deltaT_dmu_nfitdata,T_dmu_raw-T_L,'--','color', colors(2,:),'LineWidth', 2);
...
errorbar(P_abs,dmu_dmu,delta_dmu_dmu,delta_dmu_dmu,[],[], '+', 'markerSize',10,'color', colors(1,:),'LineWidth', 3);
hold on
...
mu_ref_dmu_init = mu_ref_dmu(2)-var_mu_ref_dmu(2);
for i=[1:3]
    mu_r(i) = mu_ref_dmu_init;
    mu_ref_dmu_init = mu_ref_dmu_init+var_mu_ref_dmu(2);
figure
for i1=1:length(Int)
    plot(E, ratio{i1},'--','color', colors(i1,:),'linewidth',2)
    hold on
    plot(E,dmu_T_ratio(T_dmu_raw(i1), dmu_dmu(i1), mu_r(i),E,T_L,m_e,m_h,Eg,D),'color', colors(i1,:))
    hold on
end
xlim([1.45 1.6])
