function ratio=dmu_T_ratio(T, dmu, mu_ref,E,T_L,m_e,m_h,Eg,D)
...
BF=sinh(q.*(E-(dmu+mu_ref))./(2.*k.*T))./(cosh(q.*(E-(dmu+mu_ref))./(2.*k.*T))+cosh(((m_h-m_e)./(m_h+m_e)).*(q.*(E-Eg)./(2.*k.*T))-(D./4).*log(m_h./m_e)));
BF_ref=sinh(q.*(E-mu_ref)./(2.*k.*T_L))./(cosh(q.*(E-mu_ref)./(2.*k.*T_L))+cosh(((m_h-m_e)./(m_h+m_e)).*(q.*(E-Eg)./(2.*k.*T_L))-(D./4).*log(m_h./m_e)));

ratio=(BF./BF_ref).*(exp(q.*(E-mu_ref)./(k.*T_L))-1)./(exp(q.*(E-(dmu+mu_ref))./(k.*T))-1);